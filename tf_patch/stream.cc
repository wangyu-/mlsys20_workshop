...
Stream &Stream::ThenConvolveWithAlgorithm(
    ...
    /*dnn::AlgorithmConfig algo(algorithm_config.algorithm(),algorithm_config.algorithm_no_scratch() );
  if(g_force_algo!=-1)
  {
      dnn::AlgorithmDesc aaa(g_force_algo,false);
      algo.set_algorithm(aaa); 
      //algo.set_algorithm_no_scratch((AlgorithmDesc)g_force_algo); 
  }
  else
  {
  ...
  VLOG_CALL(PARAM(input_descriptor), PARAM(input_data),
            PARAM(filter_descriptor), PARAM(filter_data),
            PARAM(convolution_descriptor), PARAM(output_descriptor),
            PARAM(output), PARAM(algorithm_config),PARAM(g_force_algo));
  //printf("<%d>\n",g_force_algo);
