...
void ExecutorState::Process(TaggedNode tagged_node, int64 scheduled_nsec) {
  ...
  while (!inline_ready.empty()) {
    ...
    char *sss=(char*)node->def().name().c_str();
    if(sss[0]=='A'&&sss[1]=='G')
    {
	g_force_algo=sss[2]-'0';
    }
    else
    {
	g_force_algo=-1;
    ...
    /*
    string sss=SummarizeNode(*node);
    char *ss=(char *)sss.c_str();
    char *ssss;
    if((ssss=strstr(ss,"xagx"))!=0)
    {
	int aaaa=-2;
	sscanf(ssss,"xagx%d",&aaaa);
	g_force_algo=aaaa;	
    }
    else
    {
    ...
    if (vlog_) {
      VLOG(1) << "Process node: " << id << " step " << params.step_id << " "
              ...
              << " device: " << device->name()<<g_force_algo;
    }
